...
COOKIE_DEFAULT_ID = 0
COOKIE_SHIFT_TYPE = 32
COOKIE_SHIFT_VLANID = 8 + COOKIE_SHIFT_TYPE
COOKIE_TYPE_MASK = 0xff
COOKIE_TYPE_ADDRESSID = 0
COOKIE_TYPE_ROUTEID = 1
COOKIE_TYPE_NIAGARAID = 2
...
REST_NIAGARA = 'niagara'
REST_NIAGARAID = "niagara_id"
REST_NIAGARA_VIP = 'vip'
REST_NIAGARA_SIP = 'sip'
REST_NIAGARA_PATTERN = 'pattern'
REST_NIAGARA_PROTO = "proto"
REST_NIAGARA_GATEWAY = 'gateway'
REST_NIAGARA_DIP = 'dip'
REST_NIAGARA_DPORT = 'dport'

PRIORITY_VLAN_SHIFT = 1000
PRIORITY_NETMASK_SHIFT = 32
PRIORITY_NIAGARA_BASE = 100
...
PRIORITY_NIAGARA_ROUTING = 6

PRIORITY_TYPE_ROUTE = 'priority_route'
PRIORITY_TYPE_NIAGARA = 'priority_niagara'
...
def get_priority(priority_type, vid=0, route=None):
    ...
    if priority_type == PRIORITY_TYPE_NIAGARA:
        priority_type = PRIORITY_NIAGARA_ROUTING
        priority = PRIORITY_NIAGARA_BASE
        log_msg = 'niagara routing'
...
class VlanRouter(object):
    ...
    def delete(self, waiters):
        ...
        for msg in msgs:
            for stats in msg.body:
                vlan_id, _ = VlanRouter._cookie_to_id(REST_VLANID, stats.cookie)
    ...
    def _cookie_to_id(id_type, cookie):
        ...
        elif id_type == REST_NIAGARAID:
            type_id = (cookie >> COOKIE_SHIFT_TYPE) & COOKIE_TYPE_MASK
            if (type_id != COOKIE_TYPE_NIAGARAID):
                return 0, False
            rest_id = cookie & UINT32_MAX
        else:
            assert id_type == REST_ROUTEID 
            type_id = (cookie >> COOKIE_SHIFT_TYPE) & COOKIE_TYPE_MASK
            if (type_id != COOKIE_TYPE_ROUTEID):
                return 0, False
            rest_id = cookie & UINT32_MAX
    ...
    def _id_to_cookie(self, id_type, rest_id):
        ...
        if id_type == REST_VLANID:
            ...
            cookie = vid + (COOKIE_TYPE_ADDRESSID << COOKIE_SHIFT_TYPE) + rest_id
        elif id_type == REST_NIAGARAID:
            cookie = vid + (COOKIE_TYPE_NIAGARAID << COOKIE_SHIFT_TYPE) + rest_id
        else:
            assert id_type == REST_ROUTEID
            cookie = vid + (COOKIE_TYPE_ROUTEID << COOKIE_SHIFT_TYPE) + rest_id
    ...
    def _get_niagara_priority(self, entry=None):
        base_priority, log_msg = self._get_priority(PRIORITY_TYPE_NIAGARA)
        real_priority = base_priority + entry.priority
        return real_priority, log_msg
    ...
    def get_data(self):
        ...
        niagara_data = self._get_niagara_data()
        ...
        if niagara_data[REST_NIAGARA]:
            data.update(niagara_data)
...
#        return self._response(data)
    ...
    def _get_niagara_data(self):
        niagara_data = []
        for key, value in self.niagara_tbl.items():
#            if value.gateway_mac is not None:
                data = {REST_NIAGARAID : value.niagara_id,
                        REST_NIAGARA : value.priority,
                        REST_NIAGARA_VIP : value.vip,
                        REST_NIAGARA_SIP : value.sip,
                        REST_NIAGARA_PATTERN : hex(value.pattern),
                        REST_NIAGARA_PROTO : value.proto,
                        REST_NIAGARA_GATEWAY : value.gateway_ip,
                        REST_NIAGARA_DIP : value.dip,
                        REST_NIAGARA_DPORT : value.dport}
                niagara_data.append(data)

        return {REST_NIAGARA: niagara_data}
    ...
    def set_data(self, data):
        ...
        try:
            ...
            # Set niagara data
            elif REST_NIAGARA in data:
                priority = int(data[REST_NIAGARA])
                vip = data[REST_NIAGARA_VIP]
                sip = data[REST_NIAGARA_SIP]
                pattern = int(data[REST_NIAGARA_PATTERN], 0)
                gateway_ip = data[REST_NIAGARA_GATEWAY]
                
                if REST_NIAGARA_DIP in data:
                    dip = data[REST_NIAGARA_DIP]
                else:
                    dip = 0
                if REST_NIAGARA_DPORT in data:
                    dport = int(data[REST_NIAGARA_DPORT])
                else:
                    dport = 0
                if REST_NIAGARA_PROTO in data:
                    proto_str = data[REST_NIAGARA_PROTO]
                    if proto_str == "tcp":
                        proto = inet.IPPROTO_TCP
                    elif proto_str == "udp":
                        proto = inet.IPPROTO_UDP
                    else:
                        msg = "Unknown ip proto in Niagara : %s" % proto_str
                        raise CommandFailure(msg = msg)
                else:
                    proto = 0

                entry = NiagaraEntry(niagara_id = 0,
                                     priority = priority,
                                     sip = sip,
                                     pattern = pattern,
                                     vip = vip,
                ...
                niagara_id = self._set_niagara_data(entry)
                details = 'Add niagara [niagara_id=%d]' % niagara_id
            # Set routing data
            ...
            msg = {REST_RESULT: REST_NG, REST_DETAILS: "missing required fields"}
            return self._response(msg)                           
    ...
    def _set_niagara_data(self, entry):
        err_msg = 'Invalid [%s] value.' % REST_NIAGARA_GATEWAY
        dst_ip = ip_addr_aton(entry.gateway_ip, err_msg=err_msg)
        address = self.address_data.get_data(ip = dst_ip)
        if address is None:
            msg = 'Gateway=%s\'s address is not registered.' % dst_ip
            raise CommandFailure(msg=msg)
        elif dst_ip == address.default_gw:
            msg = 'Gateway=%s is used as default gateway of address_id=%d'\
                % (dst_ip, address.address_id)
            raise CommandFailure(msg=msg)
        else:
            src_ip = address.default_gw
            entry = self.niagara_tbl.add(entry)
            self._set_niagara_packetin(entry)
            self.send_arp_request(src_ip, dst_ip)
            return entry.niagara_id
    ...
    def _set_niagara_packetin(self, entry):
        cookie = self._id_to_cookie(REST_NIAGARAID, entry.niagara_id)
        priority, log_msg = self._get_niagara_priority(entry)
        self.ofctl.set_wildcard_packetin_flow(cookie,
                                              priority,
                                              dl_type = ether.ETH_TYPE_IP,
                                              dl_vlan = self.vlan_id,
                                              src_ip = entry.sip,
                                              src_ip_mask = entry.pattern,
                                              dst_ip = entry.vip,
                                              ip_proto = entry.proto)
        self.logger.info('Set %s (packet in) flow [cookie=0x%x]', log_msg,
                         cookie, extra=self.sw_id)
    ...
    def delete_data(self, data, waiters):
        ...
        elif REST_NIAGARAID in data:
            niagara_id = data[REST_NIAGARAID]
            msg = self._delete_niagara_data(niagara_id, waiters)
        else:
            raise ValueError('Invalid parameter.')
    ...
    def _delete_address_data(self, address_id, waiters):
        ...
        for msg in msgs:
            for stats in msg.body:
                ...
                if vlan_id != self.vlan_id:
                    continue
                addr_id, correct_type = VlanRouter._cookie_to_id(REST_ADDRESSID,
                                                                 stats.cookie)
                if (not correct_type):
                    continue
        ...
        for flow_stats in delete_list:
            ...
            address_id, _ = VlanRouter._cookie_to_id(REST_ADDRESSID,
                                                  flow_stats.cookie)
    ...
    def _delete_routing_data(self, route_id, waiters):
        ...
        for msg in msgs:
            for stats in msg.body:
                ...
                if vlan_id != self.vlan_id:
                    continue
                rt_id, correct_type = VlanRouter._cookie_to_id(REST_ROUTEID,
                                                               stats.cookie)
                if (not correct_type):
                    continue
        ...
        for flow_stats in delete_list:
            self.ofctl.delete_flow(flow_stats)
            route_id, _ = VlanRouter._cookie_to_id(REST_ROUTEID,
                                                flow_stats.cookie)
    ...
    def _delete_niagara_data(self, niagara_id, waiters):
        if niagara_id != REST_ALL:
            try:
                niagara_id = int(niagara_id)
            except ValueError as e:
                err_msg = 'Invalid [%s] value. %s'
                raise ValueError(err_msg % (REST_NIAGARAID, e.message))
        ...
        for msg in msgs:
            for stats in msg.body:
                ...
                if vlan_id != self.vlan_id:
                    continue
                ng_id, correct_type = VlanRouter._cookie_to_id(REST_NIAGARAID,
                                                               stats.cookie)
                if (not correct_type):
                    continue
                if niagara_id == REST_ALL:
                    if ng_id == COOKIE_DEFAULT_ID:
                        continue
                elif niagara_id != ng_id:
                    continue
                delete_list.append(stats)
        ...
        for flow_stats in delete_list:
            self.ofctl.delete_flow(flow_stats)
            ng_id, _  = VlanRouter._cookie_to_id(REST_NIAGARAID,
                                             flow_stats.cookie)
            self.niagara_tbl.delete(ng_id)
            if ng_id not in delete_ids:
                delete_ids.append(ng_id)


        msg = {}
        if delete_ids:
            delete_ids = ','.join(str(ng_id) for ng_id in delete_ids)
            details = 'Delete route [niagara_id=%s]' % delete_ids
            msg = {REST_RESULT: REST_OK, REST_DETAILS: details}
    ...
    def _packetin_arp(self, msg, header_list):
        ...
        #  Learning host MAC.
        rt_flg = self._update_routing_tbl(msg, header_list)
        ng_flg = self._update_niagara_tbl(msg, header_list)
        if (not rt_flg) and not (ng_flg):
...
#            self._learning_host_mac(msg, header_list)
    ...
    def _update_niagara_tbl(self, msg, header_list):
        # Set flow: routing to gateway.
        out_port = self.ofctl.get_packetin_inport(msg)
        src_mac = header_list[ARP].src_mac
        dst_mac = self.port_data[out_port].mac
        ...
        for key, value in self.niagara_tbl.items():
            if value.gateway_ip == src_ip:
                ...
                if value.gateway_mac == src_mac:
                    continue
                self.niagara_tbl[key].gateway_mac = src_mac

                cookie = self._id_to_cookie(REST_NIAGARAID, value.niagara_id)
                priority, log_msg = self._get_niagara_priority(entry = value)
                self.ofctl.set_niagara_flow(cookie,
                                            priority,
                                            out_port,
                                            dl_vlan = self.vlan_id,
                                            src_ip = value.sip,
                                            src_ip_mask = value.pattern,
                                            dst_ip = value.vip,
                                            ip_proto = value.proto,
                                            dec_ttl=True,
                                            wrt_src_mac = dst_mac,
                                            wrt_dst_mac = src_mac,
                                            wrt_dst_ip = value.dip,
                                            wrt_dst_port = value.dport)
                self.logger.info('Set %s flow [cookie=0x%x]', log_msg, cookie,
                                 extra=self.sw_id)
        return gateway_flg
...
# store a rule that direct traffic (from Sip to Vip) to a Dip
# may rewrite Dport as well
class NiagaraEntry(object):
    def __init__(self, 
                 vip,
                 sip = 0,
                 pattern = 0,
        ...
        super(NiagaraEntry, self).__init__()
        self.niagara_id = niagara_id
...
# store niagara entries
class NiagaraTable(dict):

    def __init__(self):
        super(NiagaraTable, self).__init__()
        self.niagara_id = 0

    def add(self, entry):

        err_msg = 'Invalid [%s] value.'
        gateway_ip = ip_addr_aton(entry.gateway_ip, 
                                  err_msg=err_msg % REST_GATEWAY)

        # key is (src_ip, src_ip_mask, dst_ip, dst_ip_mask)
        key = '%s&%d-%s' % (entry.sip, entry.pattern,
                               entry.vip)
        if key in self:
            msg = 'Duplicated rule for key=%s' % key
            raise CommandFailure(msg=msg)

        ## TODO
        ## Assuming it never over flow
        self.niagara_id += 1
        self.niagara_id &= UINT32_MAX
        entry.niagara_id = self.niagara_id
        self[key] = entry

        return entry

    def delete(self, niagara_id):
        for key, value in self.items():
            if value.niagara_id == niagara_id:
                del self[key]
                return

    def get_gateways(self):
        return [entry.gateway_ip for entry in self.values()]

    def get_data(self, vip, sip):
        best_entry = None
        for entry in self.values():
            if (entry.vip == vip):
                if (ipv4_apply_wildcard(sip, entry.pattern) == entry.sip):
                    # For priority match
                    if best_entry.priority < entry.priority:
                        best_entry = entry
...
class OfCtl(object):
    ...
    ## here src and dst ip are prefix masked
    ## it is used for routing purpose, unchanged from rest_router.py
    def set_flow(self,
                 cookie,
                 priority,
                 dl_type = 0,
    ...
    def set_packetin_flow(self,
        ...
        self.set_flow(cookie, priority,
                      dl_type=dl_type, dl_dst=dl_dst, dl_vlan=dl_vlan,
                      nw_dst=dst_ip, dst_mask=dst_mask,
                      nw_proto=nw_proto, actions=actions)

    def set_wildcard_packetin_flow(self,
                                   cookie,
                                   priority,
                                   dl_type=0,
                                   dl_dst=0,
...
class OfCtl_after_v1_2(OfCtl):
    ...
    def set_flow(self,
        ...
        dst_ip_mask = UINT32_MAX
        if nw_src:
            src_ip_mask = mask_ntob(src_mask)
        if nw_dst:
            dst_ip_mask = mask_ntob(dst_mask)
    ...
    ## niagara matches on src wildcard pattern
    def set_niagara_flow(self,
                         cookie,
                         priority,
                         outport,
                         dl_vlan = 0,
        ...
        if dec_ttl:
            actions.append(ofp_parser.OFPActionDecNwTtl())
        if wrt_src_mac:
            actions.append(ofp_parser.OFPActionSetField(eth_src = wrt_src_mac))
        if wrt_dst_mac:
            actions.append(ofp_parser.OFPActionSetField(eth_dst = wrt_dst_mac))
        if wrt_dst_ip:
            actions.append(ofp_parser.OFPActionSetField(ipv4_dst = wrt_dst_ip))
        if wrt_dst_port:
            actions.append(ofp_parser.OFPActionSetField(tcp_dst = wrt_dst_port))
        if outport is not None:
            actions.append(ofp_parser.OFPActionOutput(outport, 0))
