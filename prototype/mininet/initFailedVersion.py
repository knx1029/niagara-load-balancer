...
def get_priority(priority_type, vid=0, route=None):
    ...
    if priority_type == PRIORITY_TYPE_NIAGARA:
        priority =  entry.priority + NIAGARA_SHIFT
...
class VlanRouter(object):
    ...
    def _cookie_to_id(id_type, cookie):
        if id_type == REST_VLANID:
            ...
            assert type_id == COOKIE_TYPE_ADDRESSID
            rest_id = cookie & UINT32_MAX
        elif id_type == REST_NIAGARAID:
            type_id = (cookie >> COOKIE_SHIFT_TYPE) & COOKIE_TYPE_MASK
            assert type_id == COOKIE_TYPE_NIAGARAID
            rest_id = cookie & UINT32_MAX
        else:
            assert id_type == REST_ROUTEID 
            type_id = (cookie >> COOKIE_SHIFT_TYPE) & COOKIE_TYPE_MASK
            assert type_id == COOKIE_TYPE_ROUTEID
            rest_id = cookie & UINT32_MAX
    ...
    def _get_niagara_priority(self, entry=None):
        base_priority, log_msg = _get_priority(PRIORITY_TYPE_NIAGARA)
        return base_priority + entry.priority, log_msg
        ...
        for key, value in self.niagara_tbl.items():
            if value.gateway_mac is not None:
                data = {REST_NIAGARAID: value.niagara_id,
                        REST_NIAGARA_VIP : value.dst_ip,
                        REST_NIAGARA_SIP : value.src_ip,
                        REST_NIAGARA_PATTERN : value.src_ip_mask,
                        REST_NIAGARA_PROTO : value.ip_proto,
                        REST_NIAGARA_GATEAY : gateway_ip,
                        REST_NIAGARA_DIP : value.wrt_dst_ip,
                        REST_NIAGARA_DPORT : value.dst_port}
    ...
    def set_data(self, data):
        ...
        try:
            ...
            # Set niagara data
                ...
                priority = data[REST_NIAGARA]
                vip = data[REST_NIAGARA_VIP]
                sip = data[REST_NIAGARA_SIP]
                pattern = int(data[REST_NIAGARA_PATTERN])
    ...
    def _set_niagara_data(self, entry):
        print "_set_niagara_data"
        err_msg = 'Invalid [%s] value.' % REST_NIAGARAGATEWAY
    ...
    def _delete_niagara_data(self, niagara_id, waiters):
        ...
        for msg in msgs:
            for stats in msg.body:
                ...
                if vlan_id != self.vlan_id:
                    continue
                ng_id = VlanRouter._cookie_to_id(REST_NIAGARAID, stats.cookie)
                if niagara_id == REST_ALL:
        ...
        for flow_stats in delete_list:
            self.ofctl.delete_flow(flow_stats)
            ng_id = VlanRouter._cookie_to_id(REST_NIAGARAID,
                                             flow_stats.cookie)
...
class NiagaraTable(dict):
    ...
    def add(self, priority, entry):
        ...
        self.niagara_id += 1
        self.niagara_id &= UINT32_INT
        entry.niagara_id = niagara_id
...
## for niagara's purpose, apply wildcard mask
def ipv4_apply_wildcard(address, netmask, err_msg=None):
    import itertools
    assert isinstance(address, str)
    address_int = ipv4_test_to_int(address)
    return ipv4_int_to_text(address_int & netmask)
